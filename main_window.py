...
class MarkerGUI(QMainWindow):
    ...
    _CONFIG_MAP = {
        ...
        "disable_image_extraction": ("disable_image_extraction", "isChecked", "setChecked"),
        "disable_multiprocessing": ("disable_multiprocessing", "isChecked", "setChecked"),
        ...
        "force_ocr": ("force_ocr", "isChecked", "setChecked"),
    ...
    def get_current_config(self):
        ...
        # 特殊处理项 (不在映射表中的)
        config["converter_cls"] = self.converter_cls.currentText() if hasattr(self, 'converter_cls') else ""
        config["force_layout_block"] = self.force_layout_block.text() if hasattr(self, 'force_layout_block') else ""
        
        # 高级标签页配置
        if advanced_tab:
            config["processors"] = advanced_tab.processors.text() if hasattr(advanced_tab, 'processors') else ""
            config["num_devices"] = advanced_tab.num_devices.value() if hasattr(advanced_tab, 'num_devices') else 1
            config["debug_data_folder"] = advanced_tab.debug_data_folder.text() if hasattr(advanced_tab, 'debug_data_folder') else ""
