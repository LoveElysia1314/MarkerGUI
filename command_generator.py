...
def generate_command(window):
    ...
    try:
        ...
        if window.disable_image_extraction.isChecked():
        ...
        if window.use_llm.isChecked():
            ...
            if "Vertex" in service:
                ...
                if window.vertex_location.text().strip():
                    command += f' --vertex_location "{window.vertex_location.text().strip()}"'
            elif "Ollama" in service:
                command += ' --llm_service marker.services.ollama.OllamaService'
                if window.ollama_base_url.text().strip():
                    command += f' --ollama_base_url "{window.ollama_base_url.text().strip()}"'
                ...
                if window.claude_api_key.text().strip():
                    command += f' --claude_api_key "{window.claude_api_key.text().strip()}"'
                ...
                if window.openai_api_key.text().strip():
                    command += f' --openai_api_key "{window.openai_api_key.text().strip()}"'
                ...
                if window.openai_base_url.text().strip():
                    command += f' --openai_base_url "{window.openai_base_url.text().strip()}"'
            else:  # Gemini
                if window.gemini_api_key.text().strip():
                    command += f' --gemini_api_key "{window.gemini_api_key.text().strip()}"'
        ...
        if hasattr(advanced_tab, 'processors') and advanced_tab.processors.text().strip():
            processors = advanced_tab.processors.text().strip()
            command += f' --processors "{processors}"'
        
        # 调试选项
        if hasattr(advanced_tab, 'debug_data_folder') and advanced_tab.debug_data_folder.text().strip():
            debug_folder = advanced_tab.debug_data_folder.text().strip()
            command += f' --debug_data_folder "{debug_folder}"'
        if hasattr(advanced_tab, 'debug_layout_images') and advanced_tab.debug_layout_images.isChecked():
            command += " --debug_layout_images"
        if hasattr(advanced_tab, 'debug_pdf_images') and advanced_tab.debug_pdf_images.isChecked():
            command += " --debug_pdf_images"
        if hasattr(advanced_tab, 'debug_json') and advanced_tab.debug_json.isChecked():
            command += " --debug_json"
        
        # 多GPU设置
        if hasattr(advanced_tab, 'num_devices') and advanced_tab.num_devices.value() > 1:
